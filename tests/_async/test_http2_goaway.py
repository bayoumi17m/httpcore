...
class AsyncMockStreamWithFailure(httpcore.AsyncMockStream):
    """A mock stream that can simulate different behaviors for testing retries."""
    ...
    def __init__(self, buffers_by_connection: list[list[bytes]], http2: bool = False):
        ...
        super().__init__(
            buffers_by_connection[0] if buffers_by_connection else [], http2=http2
        )
...
class MockConnectionGracefulGoaway:
    """
    ...
    def __init__(self, origin: httpcore.Origin, succeed_on_retry: bool = True):
        self._origin = origin
        self._calls = 0
        self._succeed_on_retry = succeed_on_retry
...
async def test_connection_pool_retries_graceful_shutdown_no_headers_sent():
    ...
    class TestPool(httpcore.AsyncConnectionPool):
        def __init__(self):
            super().__init__()
            self._mock_connections: list = []

        def create_connection(self, origin: httpcore.Origin):
            conn = MockConnectionGracefulGoaway(origin, succeed_on_retry=True)
    ...
    async with TestPool() as pool:
        ...
        assert pool._mock_connections[0]._calls == 2
