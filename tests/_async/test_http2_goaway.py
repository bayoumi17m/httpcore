...
@pytest.mark.anyio
async def test_mock_backend_with_retry_exhausted_buffers():
    """
    Test the AsyncMockBackendWithRetry when more connections are made
    ...
    stream1 = await network_backend.connect_tcp("example.com", 443)
    assert stream1 is not None

    # Second connection should hit the else branch (empty buffer)
    stream2 = await network_backend.connect_tcp("example.com", 443)
    assert stream2 is not None


@pytest.mark.anyio
async def test_mock_connection_graceful_goaway_info():
    """
